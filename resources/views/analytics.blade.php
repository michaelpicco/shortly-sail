                ...
                <div class="relative w-full max-w-2xl px-6 lg:max-w-7xl">
                    ...
                    <main class="mt-6 text-center">
                        <div class="grid gap-6 lg:gap-8">
                            ...
                            @isset($details)
                                <h1 class="text-4xl">Shortened URL Details</h1>
                                <table class="table table-hover table-bordered table-auto mb-16 text-center">
                                    <thead>
                                    <tr>
                                        <th scope="col"></th>
                                    </tr>
                                    ...
                                    <tbody>
                                    <tr>
                                        <td class="text-2xl">Title: {{ $details->title }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-2xl">Short URL: 
                                        <a
                                                href="/s/{{ $details->hash }}"
                                                class="rounded-md px-3 py-2 text-black ring-1 ring-transparent transition hover:text-black/70 focus:outline-none focus-visible:ring-[#FF2D20] dark:text-white dark:hover:text-white/80 dark:focus-visible:ring-white"
                                            >
                                            {{ route('home') }}/s/{{ $details->hash }}
                                            </a>     
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="text-2xl">Redirect URL: {{ $details->url }}</td>
                                    </tr>
                                    </tbody>
                                </table>
                            @endisset
                            ...
                            @isset($logs)
                            <h1 class="text-4xl">URL Logs</h1>
                            <table class="table table-hover table-bordered text-left">
                                <thead>
                                <tr>
                                    <th class="px-2 py-4" scope="col"></th>
                                    <th class="px-2 py-4" scope="col">Title</th>
                                    @empty($details)
                                    <th class="px-2 py-4" scope="col">Short URL</th>
                                    <th class="px-2 py-4" scope="col">Redirect URL</th>
                                    @endempty
                                    <th class="px-2 py-4" scope="col">IP</th>
                                    <th class="px-2 py-4" scope="col">Browser</th>
                                    <th class="px-2 py-4" scope="col">Platform</th>
                                    <th class="px-2 py-4" scope="col">Referrer</th>
                                    <th class="px-2 py-4" scope="col">Created</th>
                                </tr>
                                </thead>
                                <tbody>
                                @if (count($logs) < 1)
                                 <tr>
                                    <td class="text-center text-2xl pt-10" colspan="7">No logs found.</td>
                                 </tr>
                                @else
                                    @foreach ($logs as $log)
                                    <tr>
                                        <td class="px-2 py-4">{{ $log->id }}</td>
                                        <td class="px-2 py-4">{{$log->shorturls->title}}</td>       
                                        @empty($details)             
                                        <td class="px-2 py-4"> 
                                            @if (Route::has('analytics'))
                                                <a
                                                    href="/analytics/{{ $log->shorturls->hash }}"
                                                    class="rounded-md px-3 py-2 text-black ring-1 ring-transparent transition hover:text-black/70 focus:outline-none focus-visible:ring-[#FF2D20] dark:text-white dark:hover:text-white/80 dark:focus-visible:ring-white"
                                                >
                                                    {{ $log->shorturls->hash }}
                                                </a>                                     
                                        ...
                                        <td class="m-2">{{ $log->shorturls->url }}</td>
                                        @endempty
                                        <td class="px-2 py-4">{{ $log->ip }}</td>
                                        <td class="px-2 py-4">{{ $log->browser }}</td>
                                        <td class="px-2 py-4">{{ $log->platform }}</td>
                                        <td class="px-2 py-4">{{ $log->referrer }}</td>
                                        <td class="px-2 py-4">{{ $log->created_at }}</td>
                                    </tr>
                                    @endforeach
                                @endif
                                </tbody>
                    ...
                    <footer class="mt-20 py-16 text-center text-sm text-black dark:text-white/70">
